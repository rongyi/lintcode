...
public:
   ...
   * @return: a boolean which equals to true if the first player will win
    我们走到i时，有两种选择:
    + 取values[i]
      我们取了values[i],对手的选择有 values[i+1]或者values[i+1] + values[i+2]
   剩下的最大总value分别为DP[i+2]或DP[i+3],
  ...
  bool firstWillWin(vector<int> &values) {
    ...
    for (int i = n - 3; i >= 0; --i) {
      int val1 = values[i] +
                 std::min(i + 2 < n ? dp[i + 2] : 0, i + 3 < n ? dp[i + 3] : 0);
      int val2 = values[i] + values[i + 1] +
                 std::min(i + 3 < n ? dp[i + 3] : 0, i + 4 < n ? dp[i + 4] : 0);
