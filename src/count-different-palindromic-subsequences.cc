...
public:
  ...
  // 1]，即当前的dp值等于左边值加下边值减去左下值，
  // 因为算左边值的时候包括了左下的所有情况，而算下边值的时候也包括了左下值的所有情况，那么左下值就多算了一遍，
  // 所以要减去。而当S[i]等于S[j]的时候，情况就比较复杂了，需要分情况讨论，因为我们不知道中间还有几个和S[i]相等的值。
  // 举个简单的例子，比如"aba"和"aaa"，当i = 0, j = 2的时候，两个字符串均有S[i]
  // == S[j]，此时二者都新增两个子序列"a"和"aa"，
  // 但是"aba"中间的"b"就可以加到结果res中，而"aaa"中的"a"就不能加了，因为和外层的单独"a"重复了。我们的目标就要找到中间重复的"a"。
  // 所以我们让left = i + 1, right = j - 1，然后对left进行while循环，如果left <=
  // right,
  // 且S[left] !=
  // S[i]的时候，left向右移动一个；同理，对right进行while循环，如果left <=
  // right, 且S[right] !=  S[i]的时候，
  // left向左移动一个。这样最终left和right值就有三种情况：

  // 1. 当left >
  // righ时，说明中间没有和S[i]相同的字母了，就是"aba"这种情况，那么就有dp[i][j]
  // = dp[i + 1][j - 1] * 2 + 2，
  // 其中dp[i + 1][j -
  // 1]是中间部分的回文子序列个数，为啥要乘2呢，因为中间的所有子序列可以单独存在，也可以再外面包裹上字母a，
  // 所以是成对出现的，要乘2。加2的原因是外层的"a"和"aa"也要统计上。

  // 2. 当left =
  // right时，说明中间只有一个和S[i]相同的字母，就是"aaa"这种情况，那么有dp[i][j]
  // = dp[i + 1][j - 1] * 2 + 1，
  // 其中乘2的部分跟上面的原因相同，加1的原因是单个字母"a"的情况已经在中间部分算过了，外层就只能再加上个"aa"了。

  // 3. 当left <
  // right时，说明中间至少有两个和S[i]相同的字母，就是"aabaa"这种情况，那么有dp[i][j]
  // = dp[i + 1][j - 1] * 2 - dp[left + 1][right -
  // 1]，其中乘2的部分跟上面的原因相同，
