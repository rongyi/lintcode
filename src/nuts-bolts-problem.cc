 ...
 * When "a" is not a nut or "b" is not a bolt, it will return 2, which is not
 * valid.
 */
...
public:
  ...
  */
  ...
  void quickSort(vector<string> &nuts, vector<string> &bolts,
                 Comparator compare, int start, int end) {
    if (start >= end)
      return;
  ...
  int partition(vector<string> &norb, string pivot, Comparator compare,
                int start, int end) {
    ...
    for (int i = start; i < end; i++) {
      if (compare.cmp(norb[i], pivot) == -1 ||
        ...
        // the last position is the save place for element which is equal to
        // pivot
