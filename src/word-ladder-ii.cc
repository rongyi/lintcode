...
public:
   ...
   * @return a list of lists of string
   如果要返回所有的结果，问题变复杂了些。因为用BFS相对于DFS的劣势就是
   不方便存储结果。 这种需要返回所有结果的，还是应该从DFS考虑，但是直
   接应用DFS复杂度会很高，因为这道题我们只要知道结尾就好了，不用继续往下搜。

   所以问题就转化成怎样用DFS的同时又可以限制DFS的深度，所以我们可以BFS与DFS结合。
   先用BFS搜到结尾字符串，然后把中途所有的字符串及其跟起始字符的 edit distance
   存在一个map里。这样的话，我们就可以从结尾字符串开始DFS，只有Map内的字符串才考
   虑继续DFS，直至搜到起始字符。

   注意这里有个小技巧，就是为什么不从起始字符串开始DFS直至搜到结尾字符串，
   而是反过来。 这里可以脑子里想像一个图，如果从起始字符串开 始搜，
   到最后一层的话会有很多无效搜索， 因为那层我们只需要找到结尾字符串，
   那么多无效的搜索到最一层太浪费时间。反之，如果我们从结尾字符串
   开始DFS, 我们把起始层控制在一个字符串，整个图先越来越宽，然后
   越来越窄直到起始字符串，而非一直越来越宽直到结尾字符串那层。
  */
...
int main() {
  ...
  string start("hot");
  string end("dog");
  unordered_set<string> dict{"hot", "cog", "dog", "tot",
                             "hog", "hop", "pot", "dot"};
